...
const modelMap = {
    rt_car_access_by_device: RtCarAccessByDevice,
    rt_car_access: RtCarAccess,
    tf_waste_weights: TfWasteWeights,
    tf_waste_carbon_print: TfWasteCarbonPrint,
    tf_waste_observed_routes: TfWasteObservedRoutes,
};
...
async function parallelScan(tableName, filters, segments, operation) {
    const Model = modelMap[tableName];
    if (!Model) {
        throw new Error(`No se encontró el modelo de MongoDB para la tabla: ${tableName}`);
    }
...
// Construir filtro Mongo a partir de normalized filters
function buildMongoFilters(filters) {
    const mongoFilter = {};
    for (const [key, val] of Object.entries(filters)) {
        if (val && typeof val === 'object' && val.type) {
            if (val.type === 'between') {
                mongoFilter[key] = { $gte: val.start, $lte: val.end };
            } else if (val.type === 'begins_with') {
                // Búsqueda por prefijo (escape si hace falta según tus datos)
                mongoFilter[key] = { $regex: `^${val.value}` };
            }
        } else {
            mongoFilter[key] = val;
        }
